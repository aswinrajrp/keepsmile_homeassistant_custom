...
effect_to_scene_mapping = {
    # Effect.NONE: None,
    Effect.JUMP_7: KS05Scene.JUMP_7,
    Effect.JUMP_3: KS05Scene.JUMP_3,
    Effect.FADE_7: KS05Scene.FADE_7,
    Effect.FADE_3: KS05Scene.FADE_3,
    Effect.FLASH: KS05Scene.FLASH,
    Effect.AUTO: KS05Scene.AUTO,
}

effect_to_music_model_mapping = {
    # Effect.NONE: None,
    Effect.FADE_7_FAST_ON_NOISE: KS05MusicModel.FADE_7_FAST_ON_NOISE,
    Effect.TWO_FADE_FAST_ON_NOISE: KS05MusicModel.TWO_FADE_FAST_ON_NOISE,
    Effect.JUMP_ON_NOISE_PAUSE_QUIET: KS05MusicModel.JUMP_ON_NOISE_PAUSE_QUIET,
    Effect.JUMP_ON_NOISE_OFF_QUIET: KS05MusicModel.JUMP_ON_NOISE_OFF_QUIET,
}

class KS05Compiler(StateCompiler):
    def compile(self, state: LightState):
        ...
        # KS05 uses different commands for setting color brightness etc.
        ...
        if cmd := state.state.get('EffectCommand'):
            if cmd.effect != Effect.NONE:
                ...
                if music_model:
                    platform_commands.append(
                        KS05MusicModelCommand(music_model, speed)
                    )
                    
                if scene:
                    platform_commands.append(
                        KS05SceneCommand(scene, speed, brightness)
                    )
        ...
        platform_commands.append(
            KS05RGBWBrightnessSpeedCommand(
                red=r, 
                green=g,
                blue=b,
