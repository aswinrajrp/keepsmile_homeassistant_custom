...
class KS05SceneCommand(PlatformCommand):
    _scene_struct = Struct("BBB")

    def __init__(self, scene_num: KS05Scene, speed: int, brightness: int):
        self._bytes = b"\x5C\x00" + self._scene_struct.pack(scene_num + 128, speed, brightness) + b"\x00\xC5"

class KS05MusicModelCommand(PlatformCommand):
...
class KS05LightColorCeilingRGB_CCTCommand(PlatformCommand):
    _color_struct = Struct("BBBBBB")
...
class KS05MusicRBGBrightnessCommand(PlatformCommand):
...
class KS05MusicRGBWBrigtnessCommand(PlatformCommand):
...
class KS05WWCWCommand(PlatformCommand):
